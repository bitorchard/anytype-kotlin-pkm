...
| 8: Disambiguation Flow & Manual Input | ✅ Implemented | Fixed `AssimilationPlan` positional-arg bug (6 sites); `ChangeSet.disambiguationChoicesJson` field + Room DB v2 + `@Serializable` on all disambiguation types; `DisambiguationResolver` with 7 unit tests; multi-step Approval UI (Disambiguating → ReviewingPlan); `ResolutionFeedbackStore.recordResolution()` wired on user pick; `ManualInputScreen` + `ManualInputViewModel` + nav graph destination + Dashboard "Test input" button |
...
## Phase 8: Disambiguation Flow & In-App Manual Input

**Goal:** Close the disambiguation loop — entities the resolver could not confidently match are now
persisted inside the `ChangeSet` and surfaced as a first step in the Approval flow, where the user
picks the correct AnyType object (or "Create new"). Additionally, add a `ManualInputScreen` that
lets developers and first-time users bypass the physical Pebble ring to test the pipeline by typing
text directly. Fix a positional-argument compilation bug in `PipelineE2ETest` discovered during
Phase 7 E2E review.

**Dependencies:** Phase 2 (`ChangeSet` model), Phase 4 (`DisambiguationChoice` model,
`ResolutionFeedbackStore`), Phase 5 (`ApprovalScreen` / `ApprovalViewModel`), Phase 7 (full DI).

---

### Task 8.1: Fix `AssimilationPlan` Positional-Arg Compilation Bug

`PipelineE2ETest` calls `AssimilationPlan(operations, emptyList(), metadata(x))` with positional
arguments, but the class is defined `AssimilationPlan(operations, metadata, disambiguationChoices)`.
Six call sites must be converted to named parameters.

**File:** `pebble-assimilation/…/PipelineE2ETest.kt` (~6 lines changed)

**Acceptance criteria:**
- [x] `./gradlew :pebble-assimilation:testDebugUnitTest` compiles and all 8 tests pass.
- [x] No positional-arg call remains for `AssimilationPlan` in test sources.

---

### Task 8.2: Add `disambiguationChoices` to `ChangeSet`

`ChangeSet` currently lacks the field, so disambiguation context is dropped at persistence time.

**Changes:**

1. **`ChangeSet.kt`** — add `val disambiguationChoices: List<DisambiguationChoice> = emptyList()`
   (kotlinx.serialization handles nested serialization via existing `@Serializable` annotations on
   `DisambiguationChoice`, `ExtractedEntity`, `ScoredCandidate`, `SignalBreakdown`).

2. **Room** — `ChangeSetEntity` gains a `String` column
   `disambiguationChoicesJson TEXT NOT NULL DEFAULT ''`; `ChangeSetDao` encodes/decodes via
   `Json.encodeToString` / `Json.decodeFromString`. `PebbleChangeDatabase` version bumped to 2,
   `fallbackToDestructiveMigration()` already present so no manual migration needed for dev.

3. **`LocalChangeCache`** — mapper updated to include the new field.

4. **`AnytypeChangeStore`** — disambiguation choices serialised as a plain JSON string stored in a
   new `pkm-disambiguationChoices` relation (FORMAT: `LONG_TEXT`); added to `PkmRelation` enum
   (taxonomy version bump not required — relation is used only by Pebble code).

**New test:** `ChangeSetSerializationTest.disambiguationChoicesRoundTrip`

**Acceptance criteria:**
- [x] `ChangeSet` with `disambiguationChoices` survives a round-trip through `LocalChangeCache`.
- [x] New serialization test passes.

---

### Task 8.3: Persist Disambiguation Choices in `AssimilationEngine`

Update the `ChangeSet` construction in `AssimilationEngine.process` to include
`plan.disambiguationChoices`.

**File:** `AssimilationEngine.kt` (~4 lines)
...
// After  (add disambiguationChoices field)
val changeSet = ChangeSet(
    …
    operations = plan.operations.map { op -> op.copy(changeSetId = changeSetId) },
    disambiguationChoices = plan.disambiguationChoices,
    metadata = plan.metadata,
    …
)
```
...
**Acceptance criteria:**
- [x] Existing E2E test `auto-approve - disambiguation required falls back to pending review`
  reloads the saved `ChangeSet` and asserts `disambiguationChoices.size == 1`.

---

### Task 8.4: Add `DisambiguationResolver`

New class in `pebble-assimilation` that takes user-supplied resolutions and produces an updated
`ChangeSet` ready for execution (no more disambiguation required).

```kotlin
// pebble-assimilation/…/resolution/DisambiguationResolver.kt
class DisambiguationResolver @Inject constructor(private val planGenerator: PlanGenerator) {

    /**
     * Apply [userChoices] (localRef → ResolutionDecision) to [changeSet]:
     *   - Resolved entities: replace CREATE_OBJECT ops with SET_DETAILS on the existing object.
     *   - CreateNew entities: keep CREATE_OBJECT as-is.
     *   - Skipped entities: remove all ops that reference this localRef.
     * Returns a new ChangeSet with empty [disambiguationChoices] and re-ordered operations.
     */
    fun resolve(changeSet: ChangeSet, userChoices: Map<String, ResolutionDecision>): ChangeSet
}
```

**New test:** `DisambiguationResolverTest` — 3 scenarios (resolved, create-new, skipped).

**Acceptance criteria:**
- [x] `resolve()` with a `Resolved(objectId)` choice replaces `CREATE_OBJECT(localRef=X)` with a
  `SET_DETAILS(objectId=objectId)` operation.
- [x] `resolve()` with `CreateNew` leaves the `CREATE_OBJECT` op unchanged.
- [x] `resolve()` with `Skipped` removes all ops whose `params` reference that `localRef`.
- [x] Returned `ChangeSet.disambiguationChoices` is empty.

---

### Task 8.5: Update `ApprovalViewModel` with Multi-Step Disambiguation Flow

Add `ApprovalStep` sealed class and `resolveAndProceed(choices)` action.
...
sealed class ApprovalStep {
    ...
    /** All entities resolved; user reviews the final operation list. */
...
`load()` — if the loaded `ChangeSet.disambiguationChoices.isNotEmpty()`, initial step is
`Disambiguating`.

`resolveAndProceed(choices: Map<String, ResolutionDecision>)` — calls `DisambiguationResolver`,
updates the `ChangeSet` in the store, records feedback, transitions step to `ReviewingPlan`.

**Acceptance criteria:**
- [x] ViewModel emits `Disambiguating` step when loaded change set has pending choices.
- [x] After `resolveAndProceed`, step transitions to `ReviewingPlan` and operations are updated.

---

### Task 8.6: Update `ApprovalScreen` with Disambiguation UI

Add `DisambiguationStepContent` composable — shown when step is `Disambiguating`.

**UI layout:**
```
┌────────────────────────────────────────────────┐
│  Resolve ambiguous entities                    │  ← Section header
│                                                │
│  "Alex" (Person)                               │  ← Entity card
│  ○ Alex Smith   [Person] [0.81 ●●●●○]         │
│  ○ Alex Johnson [Person] [0.79 ●●●●○]         │
│  ○ Create new "Alex"                           │
│                                                │
│  "Friday game" (Event)                         │  ← second entity
│  ○ …                                           │
│                                                │
│          [ Apply choices ]   (disabled until   │
│                               all picked)      │
└────────────────────────────────────────────────┘
```

Each candidate card shows: object name, type label, confidence bar (1–5 filled dots based on
score), and a radio button.

**Acceptance criteria:**
- [x] `DisambiguationStepContent` renders for each `DisambiguationChoice`.
- [x] "Apply choices" button disabled until every entity has a selection.
- [x] Tapping "Apply choices" calls `viewModel.resolveAndProceed(choices)`.
- [x] Score rendered as filled/empty dot indicators (`●●●●○` for 0.80).

---

### Task 8.7: Wire `ResolutionFeedbackStore.record()` on User Pick

When `resolveAndProceed` is called, record each user choice via `ResolutionFeedbackStore` so the
`ScoringEngine` improves recency/frequency signals for future inputs.

**File:** `ApprovalViewModel.kt` (~10 lines)

```kotlin
fun resolveAndProceed(choices: Map<String, ResolutionDecision>) {
    val cs = _state.value.current ?: return
    viewModelScope.launch {
        // Record user feedback for scoring improvement
        choices.forEach { (localRef, decision) ->
            if (decision is ResolutionDecision.Resolved) {
                feedbackStore.record(localRef, decision.objectId, accepted = true)
            }
        }
        val updated = disambiguationResolver.resolve(cs, choices)
        changeStore.save(updated)          // replace the old pending change set
        _state.value = _state.value.copy(
            pendingChangeSets = _state.value.pendingChangeSets
                .map { if (it.id == cs.id) updated else it },
...
**Acceptance criteria:**
- [x] `ResolutionFeedbackStore.record()` called once per `Resolved` choice.
- [x] `Skipped` and `CreateNew` choices do not generate feedback records.

---

### Task 8.8: Add `ManualInputScreen` + `ManualInputViewModel`

A lightweight screen (accessible from the Pebble Dashboard via a "Test input" button) that lets
the user type text directly, bypassing the Pebble ring and webhook server. Enqueues a `RawInput`
with `source = "manual"` and `traceId = UUID.randomUUID()`.

**New files:**
- `feature-pebble-ui/…/manual/ManualInputScreen.kt`
- `feature-pebble-ui/…/manual/ManualInputViewModel.kt`
- `app/…/ui/pebble/ManualInputFragment.kt`
- Navigation destination added to `nav_pebble.xml`
- "Test input" `TextButton` added to `PebbleDashboardScreen`

**`ManualInputViewModel` states:**
...
`submit(text: String)` — calls `inputQueue.enqueue(RawInput(…, source = "manual"))`, transitions
to `Submitted`.

**Acceptance criteria:**
- [x] Submit button disabled when text field is blank.
- [x] Successful submission shows "Submitted — processing in background" and "Back to dashboard"
  button.
- [x] Error during `enqueue` shows error state with retry button.
- [x] Submitted input appears in `InputHistoryScreen`.
    ...
    └──→ Phase 4: Assimilation Engine                             │
             ...
             └──→ Phase 5: UI Layer ─────────────────────────────┘
                      ...
                      └──→ Phase 7: Integration & E2E Testing
                                    │
                                    └──→ Phase 8: Disambiguation Flow & Manual Input
```

**Parallelization opportunities (Phase 8):**
- Tasks 8.1 and 8.2 can proceed in parallel (independent files).
- Task 8.3 depends on 8.2 (needs the new `ChangeSet` field).
- Task 8.4 (`DisambiguationResolver`) can be written in parallel with 8.3.
- Tasks 8.5 and 8.6 depend on 8.4 (need the resolver).
- Task 8.7 can be added at the end of 8.5 (same file).
- Task 8.8 is fully independent; can proceed any time after Phase 5.
...
9. ✅ **Phase 8** (Tasks 8.1 → 8.8) — complete; `AssimilationPlan` bug fixed; `ChangeSet.disambiguationChoicesJson` + Room v2 + `@Serializable` on all disambiguation types + `ChangeSetExtensions.kt`; `DisambiguationResolver` + 7 unit tests + `DisambiguationChoicesSerializationTest`; multi-step `ApprovalViewModel` + `ApprovalStep` sealed class; `ApprovalScreen` disambiguation candidate-picker UI; `ResolutionFeedbackStore.recordResolution()` wired; `ManualInputScreen` + `ManualInputViewModel` + nav destination + Dashboard button

Total estimated tasks: **52 tasks across 9 phases** (Phase 8 adds 8 tasks: 1 bug fix, 4 model/logic, 2 UI, 1 new screen).
